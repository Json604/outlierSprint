...
import { ArrowLeft, Clock, Calendar, MapPin, Users, CreditCard, Wallet, Tag, X } from 'lucide-react';
...
export default function MovieBookingPage() {
  ...
  const { isLoggedIn, user, updateUser, appliedCoupon, setAppliedCoupon, couponDiscount, setCouponDiscount } = useUser();
  
  const [step, setStep] = useState(1); // 1: Seat/Ticket selection, 2: Checkout, 3: Confirmation
  ...
  // Determine if this is a movie or event booking
  const isEventBooking = !!eventId;
  const movie = movieId ? mockMovies.find(m => m.id === movieId) : null;
  const event = eventId ? mockEvents.find(e => e.id === eventId) : null;
  const theater = movie?.theaters.find(t => t.id === theaterId);

  // Get the item being booked (movie or event)
  const bookingItem = movie || event;

  useEffect(() => {
    if (isEventBooking) {
      logClick('page-view', '/book-movie', { eventId, showtime, date });
      // For events, set initial ticket count and price
      if (event) {
        const basePrice = event.price * selectedTickets;
        setOriginalPrice(basePrice);
        setTotalPrice(basePrice - couponDiscount);
      }
    } else {
      logClick('page-view', '/book-movie', { movieId, theaterId, showtime, date });
    }
  }, [movieId, eventId, theaterId, showtime, date, selectedTickets, event, isEventBooking, couponDiscount]);

  useEffect(() => {
    if (!isLoggedIn) {
      const redirectUrl = isEventBooking 
        ? `/book-movie?eventId=${eventId}&showtime=${showtime}&date=${date}`
        : `/book-movie?movieId=${movieId}&theaterId=${theaterId}&showtime=${showtime}&date=${date}`;
      router.push(`/login?redirect=${encodeURIComponent(redirectUrl)}`);
    }
  }, [isLoggedIn, router, movieId, eventId, theaterId, showtime, date, isEventBooking]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black flex items-center justify-center">
        <div className="text-center">
          ...
          <button
            onClick={() => router.push(isEventBooking ? '/events' : '/movies')}
            className="bg-red-600 text-white px-6 py-2 rounded-lg hover:bg-red-700"
          >
            Back to {isEventBooking ? 'Events' : 'Movies'}
          </button>
        </div>
      </div>
    );
  ...
  const handleSeatSelection = (seats: string[], price: number) => {
    ...
    setTotalPrice(price - couponDiscount);
  };
  ...
  const handleApplyCoupon = () => {
    ...
    // Check if user has selected seats/tickets
    if (isEventBooking && selectedTickets === 0) {
      setCouponError('Please select tickets before applying a coupon');
      return;
    }
    
    if (!isEventBooking && selectedSeats.length === 0) {
      setCouponError('Please select seats before applying a coupon');
      return;
    }

    const category = isEventBooking ? 'Events' : 'Movies';
    const ticketCount = isEventBooking ? selectedTickets : selectedSeats.length;
    const result = validateCoupon(couponCode.trim(), originalPrice, category, ticketCount);

    if (result.isValid) {
      setAppliedCoupon(result.code || couponCode);
      setCouponDiscount(result.discount);
      setTotalPrice(originalPrice - result.discount);
      setCouponSuccess(result.message);
      setCouponError('');
      setCouponCode('');
      logClick('coupon-applied', '/book-movie', { 
        code: result.code, 
  ...
  const handleRemoveCoupon = () => {
    ...
    setCouponSuccess('');
    setCouponError('');
    logClick('coupon-removed', '/book-movie');
  };
  ...
  const handleNextStep = () => {
    ...
    logClick('booking-step', '/book-movie', { step: step + 1, selectedSeats, selectedTickets, totalPrice });
    setStep(step + 1);
  };

  const handlePreviousStep = () => {
    logClick('booking-step', '/book-movie', { step: step - 1 });
    setStep(step - 1);
  };
  ...
  const handlePayment = async () => {
    ...
    if (isEventBooking) {
      logBooking(eventId!, [`GA${selectedTickets}`], totalPrice, '/book-movie');
    } else {
      logBooking(movieId!, selectedSeats, totalPrice, '/book-movie');
    }
    ...
    if (user) {
      const newBooking = {
        id: newBookingId,
        ...(isEventBooking ? { eventId } : { movieId, theaterId }),
        seats: isEventBooking ? [`GA${selectedTickets}`] : selectedSeats,
        showtime,
        date,
        totalPrice,
    ...
    // Clear coupon after successful booking
    setAppliedCoupon(null);
    setCouponDiscount(0);
  ...
  // Generate some occupied seats for demo (only for movies)
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">{bookingItem.title}</h1>
            <div className="flex items-center text-gray-400 text-sm space-x-4">
              <span className="flex items-center">
                <MapPin className="h-4 w-4 mr-1" />
                {isEventBooking ? event!.venue : theater?.name}
              </span>
              <span className="flex items-center">
        ...
        <div className="flex items-center justify-center text-sm text-gray-400 mb-8 space-x-8">
          <span className={step >= 1 ? 'text-red-400 font-medium' : ''}>
            {isEventBooking ? 'Select Tickets' : 'Select Seats'}
          </span>
              ...
              <div className="glass-card rounded-xl p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between">
                    <span className="text-gray-400">{isEventBooking ? 'Event' : 'Movie'}:</span>
                    <span className="font-medium text-white">{bookingItem.title}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    <span className="text-gray-400">{isEventBooking ? 'Tickets' : 'Seats'}:</span>
                    <span className="font-medium text-white">
                      {isEventBooking 
                        ? selectedTickets
                ...
                <div className="border-t border-gray-700 pt-4 mb-4">
                  <h4 className="font-medium text-white mb-3">Have a coupon?</h4>
                  {!appliedCoupon ? (
                    <div className="space-y-3">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-3 py-2 bg-gray-800/50 border border-gray-600 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent text-white placeholder-gray-400 text-sm"
                          data-testid="coupon-input"
                        />
                        <button
                          onClick={handleApplyCoupon}
                          className="bg-yellow-500 text-black px-4 py-2 rounded-lg hover:bg-yellow-400 transition-colors text-sm font-medium"
                    ...
                    <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                      <div className="flex items-center justify-between">
                        <div>
                          <p className="text-green-300 font-medium text-sm">{appliedCoupon}</p>
                          <p className="text-green-400 text-xs">Saved ₹{couponDiscount}</p>
                        </div>
                        <button
                ...
                <button
                  ...
                  disabled={isEventBooking ? selectedTickets === 0 : selectedSeats.length === 0}
            ...
            <div className="glass-card rounded-xl p-8">
              ...
              <div className="bg-gray-800/50 rounded-lg p-6 mb-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-400">{isEventBooking ? 'Event' : 'Movie'}:</span>
                    <p className="font-medium text-white">{bookingItem.title}</p>
                  </div>
                  <div>
                    <span className="text-gray-400">Venue:</span>
              ...
              <div className="flex gap-4">
                ...
                >
                  Back to {isEventBooking ? 'Tickets' : 'Seats'}
                </button>
                <button
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {appliedCoupon && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-green-400">{appliedCoupon} (-₹{couponDiscount})</span>
                    </div>
                  )}
