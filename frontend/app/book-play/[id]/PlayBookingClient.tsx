...
import { ArrowLeft, Clock, Calendar, MapPin, Users, CreditCard, Theater, Wallet } from 'lucide-react';
import { Play } from '@/lib/mockData';
import { useUser } from '@/contexts/UserContext';
import { logClick, logBooking } from '@/services/analyticsLogger';

interface PlayBookingClientProps {
  play: Play;
}

export default function PlayBookingClient({ play }: PlayBookingClientProps) {
  ...
  useEffect(() => {
    logClick('page-view', `/book-play/${play.id}`, { playTitle: play.title });
    setTotalPrice(play.price.regular * selectedTickets);
    if (play.dates.length > 0) {
      setSelectedDate(play.dates[0]);
    }
    if (play.showtimes.length > 0) {
      setSelectedShowtime(play.showtimes[0]);
    }
  }, [play, selectedTickets]);

  useEffect(() => {
    if (!isLoggedIn) {
      router.push(`/login?redirect=${encodeURIComponent(`/book-play/${play.id}`)}`);
    }
  }, [isLoggedIn, router, play.id]);

  const handleTicketChange = (tickets: number) => {
    setSelectedTickets(tickets);
    setTotalPrice(play.price.regular * tickets);
  };
  ...
  const handleNextStep = () => {
    ...
    logClick('booking-step', `/book-play/${play.id}`, { step: step + 1, selectedTickets, totalPrice });
    setStep(step + 1);
  };

  const handlePreviousStep = () => {
    logClick('booking-step', `/book-play/${play.id}`, { step: step - 1 });
    setStep(step - 1);
  };
  ...
  const handlePayment = async () => {
    ...
    logBooking(play.id, [`GA${selectedTickets}`], totalPrice, `/book-play/${play.id}`);
  ...
  const currentVenue = play.venues[selectedCity] || Object.values(play.venues)[0] || 'Venue TBA';
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-900 to-black">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-center text-sm text-gray-400 mb-8 space-x-8">
          <span className={step >= 1 ? 'text-purple-400 font-medium' : ''}>Select Tickets</span>
          <span className={step >= 2 ? 'text-purple-400 font-medium' : ''}>Payment</span>
          <span className={step >= 3 ? 'text-purple-400 font-medium' : ''}>Confirmation</span>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2">
              <div className="glass-card rounded-xl p-6">
                ...
                {/* Date Selection */}
                <div className="mb-6">
                  <h4 className="font-medium text-white mb-3">Select Date</h4>
                ...
                {/* Showtime Selection */}
                <div className="mb-6">
                  <h4 className="font-medium text-white mb-3">Select Showtime</h4>
                ...
                <div className="space-y-4">
                  <div className="border border-gray-700 rounded-lg p-4">
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        <h4 className="font-semibold text-white">Regular Tickets</h4>
                        <p className="text-gray-400 text-sm">Standard seating</p>
                      </div>
                      <div className="text-right">
                        <p className="text-xl font-bold text-white">₹{play.price.regular}</p>
              ...
              <div className="glass-card rounded-xl p-6 sticky top-24">
                ...
                <div className="space-y-3 mb-6">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-white">{play.title}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-white">{selectedShowtime}</span>
                  </div>
                  <div className="flex justify-between">
                ...
                <button
                  ...
                  disabled={selectedTickets === 0 || !selectedDate || !selectedShowtime}
                  className="w-full bg-purple-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-purple-700 disabled:bg-gray-600 disabled:cursor-not-allowed transition-colors"
            ...
            <div className="glass-card rounded-xl p-8">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-white">
                      {selectedDate && new Date(selectedDate).toLocaleDateString()} at {selectedShowtime}
                    </p>
                  </div>
                  <div>
                    <span className="text-gray-400">Tickets:</span>
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="flex-1 bg-purple-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-purple-700 disabled:bg-purple-400 transition-colors"
                  data-testid="pay-now"
                >
              ...
              <div className="bg-gray-800/50 rounded-lg p-6 mb-6">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between border-t border-gray-700 pt-2 mt-2">
                    ...
                    <span className="font-bold text-purple-400">₹{totalPrice}</span>
                  </div>
                </div>
              </div>
